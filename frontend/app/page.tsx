...
export default function AuthPilotDashboard() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080c14", color:"#e2e8f0",
      ...
      {/* Header */}
      <header style={{ position:"sticky", top:0, zIndex:50, height:60, padding:"0 32px",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:34, height:34, borderRadius:8,
            ...
            fontSize:15, fontWeight:900, color:"#fff" }}>A</div>
          <span style={{ fontSize:17, fontWeight:800, letterSpacing:3,
            background:"linear-gradient(90deg,#00d4ff,#60a5fa)",
            WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>AUTHPILOT</span>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1200, margin:"0 auto", padding:"36px 24px" }}>
        ...
        <div style={{ marginBottom:36 }}>
          <h1 style={{ fontSize:"clamp(22px,3.5vw,40px)", fontWeight:800, margin:0, letterSpacing:-1, lineHeight:1.15 }}>
            Prior Authorization,{" "}
            <span style={{ background:"linear-gradient(90deg,#00d4ff,#60a5fa,#a78bfa)",
              WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Automated.</span>
          </h1>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:14, marginBottom:32 }}>
          {stats.map((s,i) => (
            <div key={i} style={{ background:"rgba(255,255,255,0.025)", border:"1px solid rgba(0,212,255,0.08)",
                ...
                <div key={i} onClick={()=>setSelected(open?null:i)}
                  ...
                  <div style={{ marginTop:12, display:"flex", gap:7, flexWrap:"wrap" }}>
                    {[c.service, `CPT ${c.code}`, ...c.conditions].map((tag,ti) => (
                      <span key={ti} style={{ fontSize:10, color: ti===0 ? "#94a3b8" : "#475569",
                        background:"rgba(255,255,255,0.03)", padding:"3px 9px",
                        borderRadius:5, border:"1px solid rgba(255,255,255,0.05)" }}>{tag}</span>
                    ))}
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                        <div style={{ background:"rgba(0,0,0,0.35)", borderRadius:8, padding:14 }}>
                          ...
                          {[`Claim/${c.id}`,`DocumentReference/doc-${c.id}`,`Provenance/${c.id}-prov`].map((r,ri) => (
                            <div key={ri} style={{ fontSize:10, color:"#334155", padding:"3px 0",
                              borderBottom:"1px solid rgba(255,255,255,0.03)", fontFamily:"monospace" }}>✓ {r}</div>
                          ))}
                        </div>
                        <div style={{ background:"rgba(0,0,0,0.35)", borderRadius:8, padding:14 }}>
                          ...
                          {["get_patient_clinical_context","check_prior_auth_required","build_clinical_justification","submit_prior_auth_request"].map((t,ti) => (
                            <div key={ti} style={{ fontSize:10, color:"#4ade80", padding:"3px 0",
                              borderBottom:"1px solid rgba(255,255,255,0.03)", fontFamily:"monospace" }}>✓ {t}</div>
                          ))}
                        </div>
                      </div>
                    </div>
        ...
        {activeTab==="new" && (
          <div style={{ maxWidth:640 }}>
            <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(0,212,255,0.1)",
              ...
              {result && (
                <div style={{ marginTop:22, padding:18, background:"rgba(0,0,0,0.4)",
                  ...
                  {result.error ? (
                    ...
                    <>
                      ...
                      <pre style={{ fontSize:10, color:"#334155", overflow:"auto", maxHeight:180,
        ...
        {activeTab==="analytics" && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:18 }}>
            {[
              ...
              <div key={ci} style={{ background:"rgba(255,255,255,0.02)",
                ...
                <div style={{ fontSize:11, fontWeight:700, color:"#64748b", letterSpacing:1.5,
                  marginBottom:18, textTransform:"uppercase" }}>{chart.title}</div>
                {chart.items.map((item,ii) => (
                  <div key={ii} style={{ marginBottom:14 }}>
                    ...
                    <div style={{ height:5, background:"rgba(255,255,255,0.05)", borderRadius:3 }}>
                      <div style={{ height:"100%", width:`${item.value}%`, borderRadius:3,
            ...
            <div style={{ background:"rgba(0,212,255,0.03)",
              ...
              <div style={{ fontSize:11, fontWeight:700, color:"#00d4ff",
                letterSpacing:1.5, marginBottom:16, textTransform:"uppercase" }}>AuthPilot 4-Tool Pipeline</div>
        ...
        {/* Footer */}
        <footer style={{ marginTop:56, paddingTop:20,
